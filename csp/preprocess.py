...
def process_s_data(data, eeg_key='s_filtered', filter_key=None, start_t=0.5, end_t=3.5, fs=250):
    '''
    Parameter
    data:
    ...
    # If filter key exist
    if filter_key:
        s = data[filter_key][eeg_key] 
    
    # If filter key doesn't exist
    else:
        s = data[eeg_key]
        
    # If in shape (n_electrodes x samples), convert
    ...
    # True label
    y_left = np.zeros(len(pos_left)).astype(int)
    y_right = np.ones(len(pos_right)).astype(int)
